...
try:
    ...
    import pexpect, os, re, subprocess, sys
...
class state(object):
    ...
    def run_commands(self, child, kwargs):
        child.sendline('')
        ...
        for v in kwargs.names:
            ...
            while cmd_check == False:
                i = child.expect(['Import successful', 'Resource already managed by Terraform', kwargs.host_prompt])
                if   i == 0: pass
                elif i == 1: pass
            ...
            for p in ['lan_connectivity', 'san_connectivity']:
                ...
                if self.type in pkeys:
                    ...
                    for e in kwargs.imm_dict.orgs[org].policies[self.type]:
                        ...
                        kwargs.names  = names
                        kwargs = state(self.type).run_commands(child, kwargs)
        #child.close()
        return kwargs
