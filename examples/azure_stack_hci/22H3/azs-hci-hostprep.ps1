...
$username   = $ydata.active_directory.azure_stack_admin
...
$session_results = Invoke-Command $sessions -ScriptBlock {
    ...
    foreach ($item in $Using:feature_list) {
        if ($wf | Where-Object {$_.Name -eq $item}) {
            ...
            Write-Host "$($env:COMPUTERNAME) Unknown Feature '$($item)'" -ForegroundColor Red
        }
    }
...
foreach ($cluster in $ydata.clusters) {
    ...
    $domain_ou = "DC=" + $djoin
    $params = @{
        ADOUPath                   = "ou=$($ad.azure_stack_ou),$domain_ou"
        DomainFQDN                 = $ad.domain
        NamingPrefix               = $ad.azure_stack_prefix
