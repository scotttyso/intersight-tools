...
#=============================================================================
$environment_variables = @("azure_stack_subscription", "windows_administrator_password", "BLAH")
...
$session_results = Invoke-Command $sessions -ScriptBlock {
    ...
    #=========================================================================
    # Validate Remote Desktop Firewall Rule(s) are Configured
    #=========================================================================
    Write-Host "$computer_name`: Beginning Remote Desktop Network Firewall Rule(s) Configuration." -ForegroundColor Yellow
    ...
    foreach ($item in $network_firewall) {
        if ($item.Enabled -eq $true) {
            Write-Host " * $computer_name`: Successfully Configured Remote Desktop Network Firewall Rule $($item.Name)." -ForegroundColor Green
        } else {
            Write-Host " * $computer_name`: Failed on Enabling Remote Desktop Network Firewall Rule $($item.Name).  Exiting..." -ForegroundColor Red
            Return New-Object PsObject -property @{completed=$False}
        }
    }
    Write-Host "$computer_name`: Completed Remote Desktop Network Firewall Rule(s) Configuration." -ForegroundColor Yellow
    #=========================================================================
    ...
    if ($hyperv.State -eq "Disabled") { Enable-WindowsOptionalFeature -Online -FeatureName Microsoft-Hyper-V -All
...
if ($answer -eq 'Y') {
    Write-Host ""
    Write-Host "Removing 'AzStackHci.EnvironmentChecker' Module." -ForegroundColor Green
    Write-Host ""
